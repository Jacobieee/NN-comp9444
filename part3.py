...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        """
        flatten 28*28 image to 784*1, then becomes 256*1 after linear layer, then 64*1, then output 10*1.
        """
...
class CNN(nn.Module):
    ...
    def forward(self, x):
        """
        original image is 28*28*1. after conv1, it would be 24*24*10. Then apply max-pooling to get 12*12*10.
        after conv2 is 8*8*50, and after maxpool is 4*4*50.
        before go into linear layer, flatten neurons to 800*1 and it becomes 256*1 after fc1.
        Then it becomes 10*1 after fc2.
        """
...
class NNModel:
    ...
    def view_batch(self):
        ...
        for train in self.trainloader:
            ...
            # store labels.
            label = train[1]
            label = np.array(label).reshape(8,8)
            break
